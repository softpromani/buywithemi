        ...
        <form class="product-form text-start" action="{{ request('product-gallery')==1? route('admin.products.add') : route('admin.products.update',$product->id) }}" method="post"
            ...
            <div class="card mt-3 rest-part">
                ...
                <div class="card-body">
                    <div class="row">
                        ...
                        <div class="col-md-6 col-lg-4 col-xl-3">
                            <div class="form-group">
                                ...
                                <input type="text" id="hsn_sac_code" name="hsn_sac_code"
                                       class="form-control" value="{{ $product->hsn_sac_code ?? '' }}"
                        ...
                        <div class="col-md-3 col-lg-4 col-xl-3 physical_product_show" id="ean_code">
                            <div class="form-group">
                                <div class="d-flex gap-2">
                                    <label class="title-color">{{ translate('ean_code') }}</label>
                                </div>

                                <input type="text" placeholder="{{ translate('ean_code') }}" name="ean_code" value="{{ $product->ean  }}"
                                    class="form-control" required>
                            </div>
                        </div>

                        <div class="col-md-3 col-lg-4 col-xl-3 physical_product_show" id="length">
                            <div class="form-group">
                                <div class="d-flex gap-2">
                                    <label class="title-color">{{ translate('length') }}</label>
                                </div>

                                <input type="text" placeholder="{{ translate('length') }}" name="length" value="{{ $product->length ?? '' }}"
                                    class="form-control" required>
                            </div>
                        </div>

                        <div class="col-md-3 col-lg-4 col-xl-3 physical_product_show" id="width">
                            <div class="form-group">
                                <div class="d-flex gap-2">
                                    <label class="title-color">{{ translate('width') }}</label>
                                </div>

                                <input type="text" placeholder="{{ translate('width') }}" name="width" value="{{ $product->width ?? '' }}"
                                    class="form-control" required>
                            </div>
                        </div>

                        <div class="col-md-3 col-lg-4 col-xl-3 physical_product_show" id="height">
                            <div class="form-group">
                                <div class="d-flex gap-2">
                                    <label class="title-color">{{ translate('height') }}</label>
                                </div>

                                <input type="text" placeholder="{{ translate('height') }}" name="height" value="{{ $product->height ?? '' }}"
                                    class="form-control" required>
                            </div>
                        </div>
                    </div>
            ...
            <div class="card mt-3 rest-part physical_product_show">
                <div class="card-header">
                    <div class="d-flex gap-2">
                        <i class="tio-user-big"></i>
                        <h4 class="mb-0">{{ translate('product_features') }}</h4>
                    </div>
                </div>
                <div class="card-body">
                    <div class="row align-items-end">
                        <div class="col-md-12">
                            @foreach(json_decode($product->features) as $key => $feature)
                                @foreach($feature as $subKey => $subValue)
                                    <div class="input-button-container input-group mb-3 feat">
                                        <input type="text" class="form-control" name="features[{{ $subKey }}]" id="{{ $subKey }}" value="{{ $subValue }}" />
                                        <div class="input-group-append">
                                            @if($loop->parent->first && $loop->first)
                                                <a id="addli" class="btn btn-success btn-sm" href="#" role="button">
                                                    <h4 class="mb-0">+</h4>
                                                </a>
                                            @else
                                                <a id="remove" class="btn btn-danger btn-sm" href="#" role="button">
                                                    <h4 class="mt-1">-</h4>
                                                </a>
                                            @endif
                                        </div>
                        ...
                        <div class="col-md-12 addDiv mt-3"></div>
                    </div>
                </div>
            </div>
...
<script>
    $(document).ready(function() {
        var counter = {{ count(json_decode($product->features)) ?? 1 }};

        $('#addli').on('click', function(e) {
            e.preventDefault();
            var clonedElement = $(this).closest('.feat').clone(); // Clone the closest .feat element
            clonedElement.find('input').attr('id', 'features-' + counter).val(''); // Clear input value
            clonedElement.find('a#addli').attr('id', 'remove').removeClass('btn-success').addClass('btn-danger').html('<h4 class="mt-1">-</h4>');

            $('.addDiv').append(clonedElement);

            counter++;
        });

        // Handle dynamic removal of elements
