...
class SystemController extends Controller
{
    ...
    public function getChooseShippingAddressOther(Request $request): JsonResponse
    {
        ...
        if(isset($shipping['email'])){
            if(!isset(auth('customer')->user()->email)){
                User::where('id', auth('customer')->id())->update(['email' => $shipping['email']]);
            }
            if(!isset(auth('customer')->user()->name)){
                // Explode the name into an array
                $nameArray = explode(' ', $shipping['contact_person_name'], 2);

                // Get first name and last name
                $firstName = $nameArray[0];
                $lastName = isset($nameArray[1]) ? $nameArray[1] : '';

                User::where('id', auth('customer')->id())->update(['name' => $shipping['contact_person_name'],'f_name' => $firstName, 'l_name' => $lastName]);
            }
        }
        ...
        if (isset($shipping['alternate_phone'])) {
            $shippingAlternatePhoneValue = preg_replace('/[^0-9]/', '', $shipping['alternate_phone']);
            $shippingAlternatePhoneLength = strlen($shippingAlternatePhoneValue);
            if ($shippingAlternatePhoneLength < 4) {
                return response()->json([
                    'errors' => translate('The_phone_number_must_be_at_least_4_characters')
                ], 403);
            }
            if ($shippingAlternatePhoneLength > 20) {
                return response()->json([
                    'errors' => translate('The_phone_number_may_not_be_greater_than_20_characters')
                ], 403);
            }
        ...
        if ($request['billing_addresss_same_shipping'] == 'false' && isset($billing['alternate_billing_phone'])) {
            $billingAlternatePhoneValue = preg_replace('/[^0-9]/', '', $billing['alternate_billing_phone']);
            $billingAlternatePhoneLength = strlen($billingAlternatePhoneValue);
            if ($billingAlternatePhoneLength < 4) {
                return response()->json([
                    'errors' => translate('The_phone_number_must_be_at_least_4_characters')
                ], 403);
            }

            if ($billingAlternatePhoneLength > 20) {
                return response()->json([
                    'errors' => translate('The_phone_number_may_not_be_greater_than_20_characters')
                ], 403);
            }
        ...
        if (isset($shipping['save_address']) && $shipping['save_address'] == 'on') {
            $addressId = ShippingAddress::insertGetId([
                ...
                'country' => $shipping['country'],
                'phone' => $shipping['phone'],
                'alternate_phone' => $shipping['alternate_phone'],
                'latitude' => $shipping['latitude'],
                'longitude' => $shipping['longitude'],
                'email' => auth('customer')->check() ? (isset(auth('customer')->user()->email) ? auth('customer')->user()->email : $shipping['email']) : $shipping['email'],
                'is_billing' => 0,
            ]);
            ...
            $getShipping->email = $shipping['email'];
            $getShipping->phone = $shipping['phone'];
            $getShipping->alternate_phone = $shipping['alternate_phone'];
        ...
        if ($request['billing_addresss_same_shipping'] == 'false' && isset($billing['billing_method_id']) && $billingInputByCustomer) {
            ...
            if (isset($billing['save_address_billing']) && $billing['save_address_billing'] == 'on') {
                $billingAddressId = ShippingAddress::insertGetId([
                    ...
                    'email' => $billing['email'],
                    'phone' => $billing['billing_phone'],
                    'alternate_phone' => $billing['alternate_billing_phone'],
                    'email' => auth('customer')->check() ? null : $billing['billing_contact_email'],
                    'latitude' => $billing['billing_latitude'] ?? '',
                    'longitude' => $billing['billing_longitude'] ?? '',
                    'is_billing' => 1,
                ...
                $getBilling->email = $billing['billing_email'];
                $getBilling->phone = $billing['billing_phone'];
                $getBilling->alternate_phone = $billing['alternate_billing_phone'];
                ...
                $billingAddressId = ShippingAddress::insertGetId([
                    ...
                    'phone' => $billing['billing_phone'],
                    'alternate_phone' => $billing['alternate_billing_phone'],
                    'email' => auth('customer')->check() ? (isset(auth('customer')->user()->email) ? auth('customer')->user()->email : $billing['billing_email']) : $billing['billing_email'],
                    'latitude' => $billing['billing_latitude'] ?? '',
                    'longitude' => $billing['billing_longitude'] ?? '',
                    'is_billing' => 1,
                ]);
