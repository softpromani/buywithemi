...
class HomeController extends Controller
{
    ...
    public function default_theme(): View
    {
        ...
        $flashDeal = Cache::remember("flash_deal_{$userId}", 60, function() use ($userId) {
        ...
        $topVendorsList = Cache::remember('top_vendors_list', 60, function() {
            return Shop::active()
        ...
        $inhouseProducts = Cache::remember('inhouse_products', 60, function() {
        ...
        $inhouseShop->orders_count = Cache::remember('admin_order_count', 60, function() {
            return Order::where(['seller_is' => 'admin'])->count();
        });
        ...
        $featuredProductsList = Cache::remember('featured_products_list', 60, function() {
            return ProductManager::getPriorityWiseFeaturedProductsQuery(query: $this->product->active(), dataLimit: 10);
        });

        $latest_products = Cache::remember('latest_products', 60, function() {
            return $this->product->with(['reviews'])->active()->orderBy('id', 'desc')->take(10)->get();
        });

        $newArrivalProducts = Cache::remember('new_arrival_products', 60, function() {
            return ProductManager::getPriorityWiseNewArrivalProductsQuery(query: $this->product->active(), dataLimit: 8);
        });

        $brands = Cache::remember('brands', 60, function() {
            return Brand::active()->take(15)->get();
        });

        $bestSellProduct = Cache::remember('best_selling_products', 60, function() {
        ...
        $topRated = Cache::remember('top_rated_products', 60, function() {
        ...
        $deal_of_the_day = Cache::remember('deal_of_the_day', 60, function() {
            return DealOfTheDay::join('products', 'products.id', '=', 'deal_of_the_days.product_id')
        ...
        $recommendedProduct = Cache::remember('recommended_product', 60, function() {
