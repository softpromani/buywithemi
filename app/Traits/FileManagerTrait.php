...
trait FileManagerTrait
{
     ...
     * @return string
     */
    protected function upload(string $dir, string $format, $image = null, $extension=null): string
    {
        $storage = config('filesystems.disks.default') ?? 'public';

        if (!is_null($image)) {
            $ext = ($extension == null) ? $image->getClientOriginalExtension() : $extension;
            $isOriginalImage = in_array($ext, ['gif', 'svg']);
            if ($isOriginalImage) {
