...
if (!function_exists('getPriceRangeWithDiscount')) {
    function getPriceRangeWithDiscount(array|object $product): float|string
    {
        ...
        if ($product->discount > 0) {
            ...
            // dd($productUnitPrice);
            return '<span class="discounted_unit_price ">' . $productDiscountedPrice . '</span>' . '<del class="total_unit_price text-muted ">' . webCurrencyConverter(amount: $productUnitPrice) . '</del>';
        } else {
            return '<span class="discounted_unit_price ">' . webCurrencyConverter(amount: $productUnitPrice) . '</span>';
        }
    }
}
